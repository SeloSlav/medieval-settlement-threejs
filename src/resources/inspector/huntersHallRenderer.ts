...
export function renderHuntersHallInspector(
  ...
  const nearestGame = context.worldQueries.findNearestForagingWithRemaining(
    building.x,
    building.z,
    building.workRadius,
    'game',
  );
  const active = building.assignedLabor > 0 && nearestGame != null;
  ...
  return {
    ...
    statusText: building.assignedLabor === 0
      ? 'Idle — assign labor to hunt game'
      : nearestGame
        ? `Hunting — ${Math.round(nearestGame.remaining)} game left at trail`
        : 'Idle — no game in range',
