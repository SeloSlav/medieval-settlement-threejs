...
function renderFrontierSecurityRows(
  ...
  return `
    ...
    <li><span>Watch districts</span><span>${coverage}% of weighted holdings watched · ${security.staffedWatchtowers} staffed ${security.staffedWatchtowers === 1 ? 'watchtower' : 'watchtowers'} · command recruited companies in the field to meet an attack</span></li>
...
export function renderSettlementGrainRows(plan: SettlementGrainPlan): string {
  ...
  const attentionLabel = plan.firstAttentionKind === 'seed'
    ...
    : plan.firstAttentionKind === 'winter-fodder'
      ? 'first feed-oat input shortfall'
  ...
  return `
    ...
    <li><span>Protected grain</span><span>Seed ${Math.round(plan.seed.protected)} / ${Math.ceil(plan.seed.target)} · feed-workshop oats ${Math.round(plan.winterFodder.protected)} / ${Math.ceil(plan.winterFodder.target)} · central reserve ${Math.round(plan.granaryReserve.protected)} / ${Math.ceil(plan.granaryReserve.target)}${attention}</span></li>
...
function renderSettlementTextileRoadRow(plan: SettlementTextilePlan): string {
  ...
  const inspect = roads.firstExposedResidenceId !== null
    ...
    : roads.firstImbalancedBuildingId !== null
      ? ` <button type="button" class="inspector-jump-button" data-inspect-building="${roads.firstImbalancedBuildingId}" aria-label="Inspect first imbalanced textile branch">Inspect chain</button>`
      : '';
  ...
  return `
    <li><span>Clothing roads</span><span>${roads.matchedBranches} / ${roads.activeBranches} active branches complete raw fibre → preparation → weaving (${roads.fleeceBranches} wool · ${roads.flaxBranches} flax · ${roads.spinnerBranches} fibre-processing · ${roads.loomBranches} loom) · ${roads.roadMatchedAnnualClothPotential.toFixed(1)} / ${plan.annualClothPotential.toFixed(1)} clothing/year physically paired${fragmentation}${householdCoverage}${exposure}${exportable}${inspect}</span></li>
    ${currentServiceRow}
  `;
}
...
export function renderSettlementTextileRows(plan: SettlementTextilePlan): string {
  ...
  const unavailableCloth = plan.unavailableHouseholdClothStock > 0.05
    ...
    : '';
  const storesRow = `<li><span>Textile stores</span><span>${Math.round(plan.woolStock)} wool${plan.woolInTransit > 0.05 ? ` (${Math.round(plan.woolInTransit)} on carts)` : ''} · ${Math.round(plan.flaxStock)} flax${plan.flaxInTransit > 0.05 ? ` (${Math.round(plan.flaxInTransit)} on carts)` : ''} · ${Math.round(plan.yarnStock)} yarn${plan.yarnInTransit > 0.05 ? ` (${Math.round(plan.yarnInTransit)} on carts)` : ''} · ${Math.round(plan.linenStock)} linen${plan.linenInTransit > 0.05 ? ` (${Math.round(plan.linenInTransit)} on carts)` : ''} · ${Math.round(plan.clothStock)} clothing${plan.clothInTransit > 0.05 ? ` (${Math.round(plan.clothInTransit)} on carts)` : ''} · ${Math.round(plan.serviceableHouseholdClothStock)} serviceable to current households${plan.householdClothInTransit > 0.05 ? ` including ${Math.round(plan.householdClothInTransit)} approaching homes` : ''}${unavailableCloth} · ${formatProvisionRunway(plan.clothReserveRunwayDays)} weakest household clothing reserve</span></li>`;
  const fireRow = plan.fireDisabledSheepHoldings
    + plan.fireDisabledSpinners
    + plan.fireDisabledWeavers
    + plan.fireDisabledProsperousHomes === 0
    ? ''
    : `<li><span>Clothing fire outages</span><span>${plan.fireDisabledSheepHoldings} sheep ${plan.fireDisabledSheepHoldings === 1 ? 'holding' : 'holdings'} + ${plan.fireDisabledSpinners} fibre ${plan.fireDisabledSpinners === 1 ? 'workshop' : 'workshops'} + ${plan.fireDisabledWeavers} ${plan.fireDisabledWeavers === 1 ? 'Weaver' : 'Weavers'} + ${plan.fireDisabledProsperousHomes} prosperous ${plan.fireDisabledProsperousHomes === 1 ? 'home' : 'homes'} offline · ${Math.round(plan.fireQuarantinedClothStock)} clothing at affected workshops, cupboards, or approaching carts unavailable until recovery</span></li>`;
  if (plan.sheepHoldings === 0) {
    return `
      <li><span>Annual wool clip</span><span>No completed sheep holding</span></li>
      <li><span>Flax route</span><span>${Math.round(plan.flaxStock)} harvested fibre owned · Spinning & Retting House water turns it into linen before weaving</span></li>
      <li><span>Prepared fibre</span><span>${Math.round(plan.yarnStock)} yarn + ${Math.round(plan.linenStock)} linen owned · ${plan.annualSpinnerIntermediateCapacity.toFixed(1)} / year installed preparation ceiling</span></li>
      ${fireRow}
      ${storesRow}
  ...
  return `
    ...
    ${fireRow}
    ${storesRow}
    <li><span>Prepared fibre</span><span>${plan.annualSpinnerIntermediateCapacity.toFixed(1)} yarn or linen/year installed ceiling · ${Math.round(plan.yarnStock)} yarn + ${Math.round(plan.linenStock)} linen already physical · flax retting requires supplied water</span></li>
    <li><span>Clothing chain</span><span>${plan.annualClothPotential.toFixed(1)} clothing/year complete-chain ceiling from projected wool or physical flax through fibre preparation and Weaver labor vs ${plan.annualHouseholdClothDemand.toFixed(1)} household need · ${annualBalance} over ${TEXTILE_PLAN_DAYS_PER_YEAR} days · ${textileChainBalanceLabel(plan)}</span></li>
    ${roadRow}
  `;
}
...
export function renderTownHallInspector(
  ...
  const livestockFodderRows = livestockFodder.holdingCount === 0
    ...
    : `
      ...
      ${livestockDairyRows}
      <li><span>Winter hay coverage</span><span>${Math.round(livestockFodder.hayStock)} local hay stored · ${Math.round(livestockFodder.projectedHayStock)} projected at winter / ${Math.ceil(livestockFodder.winterHayNeed)} needed${livestockFodder.winterHayShortfall > 0.05 ? ` · short ${Math.ceil(livestockFodder.winterHayShortfall)} before Animal Feed` : ''}</span></li>
      <li><span>Winter Animal Feed</span><span>${Math.round(livestockFodder.winterReserveStock)} / ${Math.ceil(livestockFodder.winterReserveTarget)} ready onsite for the need remaining after local hay and winter pasture/mast capacity${livestockFodder.winterReserveShortfall > 0.05 ? ` · short ${Math.ceil(livestockFodder.winterReserveShortfall)} across ${livestockFodder.shortHoldings} holdings · first combined coverage ${formatProvisionRunway(livestockFodder.firstRunwayDays)}${livestockFodder.firstShortBuildingId ? ` <button type="button" class="inspector-jump-button" data-inspect-building="${livestockFodder.firstShortBuildingId}" aria-label="Inspect first winter Animal Feed shortfall">Inspect</button>` : ''}` : ' · stocked to holding targets'}</span></li>
      <li><span>Feed preparation</span><span>${Math.round(livestockFodder.oatInputStock)} / ${Math.ceil(livestockFodder.oatInputTarget)} oats onsite for pending conversion${livestockFodder.oatInputShortfall > 0.05 ? ` · input short ${Math.ceil(livestockFodder.oatInputShortfall)}` : ''} · ${LIVESTOCK_FEED_OAT_GRAIN_PER_CYCLE} oat → ${LIVESTOCK_ANIMAL_FEED_PER_CYCLE} Animal Feed at staffed pastoral farmsteads · ${renderResourceAmount('animalFeed', livestockFodder.feedConversionPerDay, { compact: true, suffix: '/day current capacity' })} · ${Math.round(livestockFodder.projectedAnimalFeedStock)} projected ready onsite before transfers</span></li>
      <li><span>Winter feeding logistics</span><span>${renderResourceAmount('animalFeed', livestockFodder.winterFeedNeed, { compact: true, suffix: `after projected local hay and winter pasture/mast capacity for ${LIVESTOCK_WINTER_FODDER_RESERVE_DAYS} days` })} · ${renderResourceAmount('animalFeed', livestockFodder.winterFeedPerDay, { compact: true, suffix: '/day after hay is gone' })} · pastoral carts can supply finished feed to swineherds${livestockFodder.capacityLimitedHoldings > 0 ? ` · ${livestockFodder.capacityLimitedHoldings} holdings need winter resupply even when full` : ''}</span></li>
    `;
  ...
  return {
    ...
    detailsHtml: `
      ...
      <li><span>Pottery chain</span><span>${industrialMaterials.potteryOutputPerDay.toFixed(1)} road-supplied / ${industrialMaterials.potterInstalledOutputPerDay.toFixed(1)} installed pottery per day &middot; ${industrialMaterials.potteryCoveredDemandPerDay.toFixed(1)} / ${industrialMaterials.potteryDemandPerDay.toFixed(1)} household demand covered &middot; ${industrialMaterials.potteryExportSurplusPerDay.toFixed(1)} exportable and ${industrialMaterials.potteryStrandedPerDay.toFixed(1)} stranded surplus &middot; consumes ${renderResourceCost({ clay: industrialMaterials.potterClayPerDay, firewood: industrialMaterials.potterFirewoodPerDay, water: industrialMaterials.potterWaterPerDay }, { compact: true, suffix: '/day' })} against ${industrialMaterials.clayOutputPerDay.toFixed(1)} sustainable clay capacity once downstream carts reopen held yards</span></li>
      <li><span>Roof-tile firing</span><span>${industrialMaterials.roofTilesOutputPerDay.toFixed(1)} road-supplied / ${industrialMaterials.potterInstalledRoofTilesPerDay.toFixed(1)} installed tiles per day &middot; each kiln assigned to tiles stops supplying market pottery until its firing recipe changes</span></li>
      ...
      <li><span>Civilian tool upkeep</span><span>${industrialMaterials.toolMaintainedSites} / ${industrialMaterials.toolEligibleSites} active racks currently equipped &middot; expected maintained uptime ${Math.round(industrialMaterials.sustainableToolUptime * 100)}% (${renderResourceAmount('ironwork', industrialMaterials.sustainableToolIronworkPerDay, { compact: true })} / ${renderResourceAmount('ironwork', industrialMaterials.fullToolIronworkPerDay, { compact: true, suffix: '/day runnable demand' })} after cart absences, geology, weather, inputs, and yard stalls) &middot; road-cart ceiling ${industrialMaterials.toolDeliveryCapacityPerDay.toFixed(2)}/day, ${industrialMaterials.toolCartWorkerDaysPerDay.toFixed(2)} smith worker-days spent hauling, ${industrialMaterials.toolUnreachableSites} unreachable sites &middot; racks reorder below ${(CIVILIAN_TOOL_IRONWORK_PER_CYCLE * CIVILIAN_TOOL_REORDER_CYCLES).toFixed(2)} and take about ${industrialMaterials.toolRefillLoad.toFixed(2)} per ordinary refill &middot; target-held sites wear nothing until work resumes &middot; post-haul smithy output counted as surplus before carpentry ${industrialMaterials.ironworkProducedSurplusPerDay.toFixed(2)}; future upkeep remains separate demand and can require a refill from shared stock${toolDeliveryInspect}${unmaintainedToolInspect}</span></li>
      ...
      <li><span>Bread capacity</span><span>${production.breadFoodCapacityPerDay.toFixed(1)} averaged bread/day vs ${provisioning.averageFreshFoodDemandPerCalendarDay.toFixed(1)} average calendar-day fresh demand after ${provisioning.householdPreservedFoodRotationPerDay.toFixed(1)} cured-ration displacement · ordinary-day fresh demand remains ${provisioning.totalFoodPerDay.toFixed(1)} from ${provisioning.grossFoodDemandPerDay.toFixed(1)} gross meals · needs ${production.breadGrainPerDay.toFixed(1)} matching rye/maslin grain plus ${renderResourceCost({ water: production.breadWaterPerDay, firewood: production.breadFirewoodPerDay }, { compact: true, suffix: '/day' })}</span></li>
      <li><span>Beverage capacity</span><span>${production.aleOutputPerDay.toFixed(1)} ale, cider, or mead / day vs ${production.aleDemandPerDay.toFixed(1)} tier-2+ demand · reflects each Brewery's selected recipe · ale uses two workshop cycles per batch and needs ${renderResourceCost({ barley: production.aleBarleyPerDay, water: production.aleWaterPerDay, firewood: production.aleFirewoodPerDay }, { compact: true, suffix: '/day' })}; cider and mead draw apples, pears, or honey directly</span></li>
      ...
      <li><span>Fibre preparation</span><span>${production.spinnerIntermediateCapacityPerDay.toFixed(1)} yarn or linen/day installed · wool route needs ${renderResourceAmount('wool', production.clothWoolPerDay, { compact: true, suffix: '/day' })}; flax route needs ${renderResourceCost({ flax: production.clothFlaxPerDay, water: production.clothFlaxWaterPerDay }, { compact: true, suffix: '/day' })}</span></li>
      <li><span>Clothing capacity</span><span>${production.clothOutputPerDay.toFixed(1)} complete-chain clothing/day from ${production.weaverClothCapacityPerDay.toFixed(1)} Weaver capacity vs ${production.clothDemandPerDay.toFixed(1)} tier-2+ demand · consumes either ${renderResourceAmount('yarn', production.clothYarnPerDay, { compact: true, suffix: '/day' })} or ${renderResourceAmount('linen', production.clothLinenPerDay, { compact: true, suffix: '/day' })}</span></li>
      <li><span>Household pottery</span><span>${production.potteryOutputPerDay.toFixed(1)} / day road-local clay-backed kiln output vs ${production.potteryDemandPerDay.toFixed(1)} tier-4 breakage replacement · local storage is filled before regional export</span></li>
