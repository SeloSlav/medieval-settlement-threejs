...
export function renderLivestockBuildingInspector(
  ...
  const weightedHealth = totalHead > 0
    ? herds.reduce((sum, herd) => sum + herd.health * herd.headCount, 0) / totalHead
    : 0;
  const waterPerCycle = herds.reduce(
    (sum, herd) => sum + livestockWaterRequiredPerCycle(herd.species, herd.headCount),
    0,
  );
  ...
  const unhealthy = totalHead > 0 && weightedHealth < 0.45;
  ...
  const statusText = (() => {
    ...
    if (herds.length === 0) return building.kind === 'swineherd'
      ...
      : 'Select a pasture to choose cattle or sheep';
    ...
    if (underSupplied) return `${(totalHead - totalSupplied).toFixed(1)} head unsupported across linked pastures`;
    if (unhealthy) return 'One or more pasture herds are recovering';
    return `${totalHead} animals tended across ${stockedHerds.length} pasture${stockedHerds.length === 1 ? '' : 's'}`;
  })();
  ...
  const pastureControls = `<div class="inspector-action-panel" data-inspector-panel-title="Pastures">
      <p class="resource-inspector-note">Draw independent livestock parcels inside this holding's work extent. Select a finished pasture to choose its animals, buy or sell that herd, set its breeding reserve, and reserve its hay meadow.</p>
  ...
  const speciesRows = summaries
    .filter(([, summary]) => summary.pastureCount > 0)
    .map(([species, summary]) => `<li><span>${SPECIES_LABEL[species][0]!.toUpperCase()}${SPECIES_LABEL[species].slice(1)}</span><span>${summary.headCount} head · ${summary.pastureCount} pasture${summary.pastureCount === 1 ? '' : 's'} · ${summary.suppliedCapacity.toFixed(1)} fully supplied / ${summary.pastureCapacity.toFixed(1)} grazing now</span></li>`)
    .join('');
  ...
  return {
    ...
    detailsHtml: `
      ...
      ${speciesRows}
      <li><span>Combined stocking</span><span>${totalHead} head · ${totalSupplied.toFixed(1)} fully supplied · ${totalPastureCapacity.toFixed(1)} grazing/mast-supported now</span></li>
