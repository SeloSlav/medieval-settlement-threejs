...
export function renderSalvagePileInspector(
  ...
  const activeHaul = activeTrip
    ...
    : 'None';
  const connected = !roadAccess.toLocaleLowerCase().includes('no road');
  const status = activeTrip
    ...
    : totalStock <= 1e-6
      ...
      : connected
        ...
        : ['Stranded · connect this site to the road network', 'warning'] as const;
  ...
  return {
    ...
    detailsHtml: `
      ...
      ${building.gold > 1e-6 ? '<li><span>Treasury recovery</span><span>Requires a road-linked Town Hall or the founding lockbox</span></li>' : ''}
    `,
