...
export function renderChapelInspector(
  ...
  const collectPanelHtml = `
    <div class="inspector-action-panel" data-inspector-panel-title="Parish orders">
      ...
      <p class="inspector-action-panel__hint">Sabbath pauses ordinary work and new cart departures for +${Math.round(CHAPEL_SABBATH_OBSERVANCE_ATTENDANCE_BONUS * 100)}% attendance and +${Math.round(CHAPEL_SABBATH_OBSERVANCE_SETTLEMENT_BONUS * 100)}% settlement speed. Households keep consuming provisions and service shortage clocks continue.</p>
