...
export function renderMarketStallsInspector(
  ...
  return {
    ...
    detailsHtml: `
      ...
      <li><span>Capacity rule</span><span>${MARKETPLACE_FOOD_STALL_SLOTS + MARKETPLACE_GOODS_STALL_SLOTS} tables fit here: ${MARKETPLACE_FOOD_STALL_SLOTS} food + ${MARKETPLACE_GOODS_STALL_SLOTS} goods · one depot worker occupies one table at one nearest Marketplace and sells one stocked need category · backyard surplus reuses the staffed group without taking another table or worker · available stock still limits each issue</span></li>
