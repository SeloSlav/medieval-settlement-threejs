...
export function createRiverWaterMesh(
  ...
  const shoreMaps = createRiverWaterShoreMaps(riverField,{terrain});
  ...
  const bedHeights = new Float32Array(nx * nz);
  ...
  const sampleStillSurface = (gx: number, gz: number): number => {
    if (
      ...
      return sampleFloatGridBilinear(bedHeights, nx, nz, gx, gz)
  ...
  // Shader displacement is invisible to CPU frustum culling.
  ...
  mesh.name = waterProfile.id === 'coastal'
    ? 'Coastal sea surface'
    : riverField.layout.corridors.length === 0
      && riverField.layout.inlandWaterBodies.length > 0
      ? 'Inland pond and lake surface'
      : 'River water surface';
