...
export function rasterizeCommunityReach(
  ...
  const rgba = colorizeCommunityReach(cells, settlementIds, resolution);
  return { resolution, settlementIds, cells, rgba };
}
  ...
  for (let row = 0; row < resolution; row += 1) {
    ...
    for (let column = 0; column < resolution; column += 1) {
      ...
      const color = COMMUNITY_REACH_PALETTE[
        stablePaletteIndex(settlementIds[settlementIndex] ?? String(settlementIndex))
      ];
...
export function stableCommunityPaletteIndex(settlementId: string): number {
  return stablePaletteIndex(settlementId);
}
