...
export function mirroredTerrainNormalParity(value: number): 1 | -1 {
  const half = value * 0.5;
  const halfFraction = half - Math.floor(half);
  return halfFraction >= 0.5 ? -1 : 1;
}
...
function reorientTerrainNormalSample(
  sample: TslNode,
  sourceUv: TslNode,
  m00: number,
  m01: number,
  ...
  const decoded = sample.rgb
    .mul(float(2) as TslNode)
    .sub(vec3(1, 1, 1) as TslNode) as TslNode;
  const mirrorParity = vec2(
    (float(1) as TslNode).sub(
      (step(
        float(0.5) as TslNode,
        fract(sourceUv.x.mul(float(0.5) as TslNode)) as TslNode,
      ) as TslNode).mul(float(2) as TslNode),
    ),
    (float(1) as TslNode).sub(
      (step(
        float(0.5) as TslNode,
        fract(sourceUv.y.mul(float(0.5) as TslNode)) as TslNode,
      ) as TslNode).mul(float(2) as TslNode),
    ),
  ) as TslNode;
  const mirroredX = decoded.x.mul(mirrorParity.x) as TslNode;
  const mirroredY = decoded.y.mul(mirrorParity.y) as TslNode;
  const reoriented = vec3(
    mirroredX.mul(float(m00) as TslNode).add(mirroredY.mul(float(m01) as TslNode)),
    mirroredX.mul(float(m10) as TslNode).add(mirroredY.mul(float(m11) as TslNode)),
    decoded.z,
  ) as TslNode;
  return reoriented
    .mul(float(0.5) as TslNode)
    .add(vec3(0.5, 0.5, 0.5) as TslNode) as TslNode;
}
...
function buildGrassBlendNodes(
  ...
  const forestBlendAttribute = attribute('forestBlend', 'float') as TslNode;
  const forestBlend = smoothstep(
    float(0.04) as TslNode,
    float(0.96) as TslNode,
    forestBlendAttribute,
  ) as TslNode;
  // The litter begins just inside the forest boundary. The original forest
  // coverage remains unchanged for canopy shade, ecology, and vegetation.
  const forestGroundBlend = smoothstep(
    float(FOREST_GROUND_TEXTURE_BLEND_START) as TslNode,
    float(FOREST_GROUND_TEXTURE_BLEND_END) as TslNode,
    forestBlendAttribute,
  ) as TslNode;
  ...
  // registered with albedo and roughness in every positive or negative tile.
  const meadowNormal = reorientTerrainNormalSample(
    texture(textures.meadow.normal, meadowUv) as TslNode,
    meadowUv,
    1,
    0,
    0,
  ...
  const denseNormal = reorientTerrainNormalSample(
    texture(textures.dense.normal, denseUv) as TslNode,
    denseUv,
    0.81232,
    0.58321,
    -0.58321,
  ...
  const dryNormal = reorientTerrainNormalSample(
    texture(textures.dry.normal, dryUv) as TslNode,
    dryUv,
    0.68569,
    -0.72789,
    0.72789,
  ...
  const normalNode = normalize(
    mix(grassNormalNode, forestNormalNode, forestGroundBlend) as TslNode,
  ) as TslNode;
...
export function createTerrainGrassMaterialWithRiverShore(
  ...
  const wornMask = max(max(shoreBlend, roadWear) as TslNode, quarryPad) as TslNode;
  const forestCanopyBlend = blendNodes.forestBlend.mul(
    sub(float(1) as TslNode, wornMask) as TslNode,
  ) as TslNode;
  const forestSurfaceBlend = blendNodes.forestGroundBlend.mul(
    sub(float(1) as TslNode, wornMask) as TslNode,
  ) as TslNode;
  ...
  const canopyGroundShade = canopyInterior
    ...
    .mul(forestCanopyBlend) as TslNode;
  ...
  // Apply the inset forest surface last so its feathered boundary resolves
  // toward overview grass and close dirt before litter reaches the interior.
