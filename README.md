...
- **Building storage** — the village storehouse centralizes timber, stone, and firewood and provides much larger construction-cart batches when staffed; it deliberately does not store food. Food stays in capped producer/processor inventories (including fishing camps), in residence need stock after delivery, or in the player treasury as fallback/overflow. Construction spending protects reserved stock from trade and other orders until it is delivered or the site is cancelled.
...
- **Learnable residence progression** — each residence costs timber and stone on placement; narrow and wide parcels get 2 or 4 population capacity respectively (default 3). Tier 1 households need only food and receive a long shortage grace period. Tier 2 adds firewood and water; tier 3 adds preserved food and ale, with increasingly strict shortage pressure and road-linked supply requirements.
...
- `npm run test:fishing` validates fish world placement, dry-land camp placement, storage, residence food claims, icon treatment, finite population harvesting, spring reproduction, winter freeze, extinction, population-scaled school visuals, and the rigged swimming/flop animation asset.
- `npm run test:foraging-ecology` validates mushroom world placement and deep-forest bias, seasonal policy, forager compatibility, persistent lifecycle wiring, herd visuals/migration, and granary collection.
