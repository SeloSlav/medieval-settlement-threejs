...
OUT_BLEND = OUT_DIR / "hunters_camp_textured_v3.blend"
OUT_GLB = OUT_DIR / "hunters_camp_textured_v3.glb"
OUT_MANIFEST = OUT_DIR / "hunters_camp_assembly_v3.json"
OUT_HERO = RENDER_DIR / "hunters_camp_hero_v3.png"
OUT_OVERHEAD = RENDER_DIR / "hunters_camp_overhead_v3.png"
OUT_WORKSIDE = RENDER_DIR / "hunters_camp_workside_v3.png"
OUT_TENT_DETAIL = RENDER_DIR / "hunters_camp_tent_detail_v3.png"
...
def assemble_camp() -> None:
    ...
    place("site_canopy_canvas_4m_d2m", "HC_Processing_Canopy", SHELTERS, (2.20, 1.75, 0.0), math.radians(4.0))
    ...
    place("prop_tool_rack_hunter", "HC_Hunter_Tool_Rack", WORK, (1.15, -2.60, 0.0), math.radians(-7.0))
    ...
    for index, (location, rotation) in enumerate((
        ...
        place("enclosure_split_rail_2m", f"HC_Boundary_Rail_{index + 1:02d}", BOUNDARY, location, rotation)
...
def stage_preview() -> bpy.types.Object:
    ...
    key = add_light("HC_Key", "AREA", (-6.0, -7.0, 11.0), 980.0, (1.0, 0.80, 0.58), 7.5)
    point_at(key, (0.0, 0.7, 0.8))
    fill = add_light("HC_Fill", "AREA", (8.0, -1.0, 6.0), 420.0, (0.54, 0.68, 1.0), 6.0)
    point_at(fill, (0.2, 1.0, 0.9))
    rim = add_light("HC_Rim", "AREA", (-4.0, 8.0, 8.0), 680.0, (0.74, 0.84, 1.0), 5.0)
    point_at(rim, (0.0, 1.4, 1.1))
    tent_bounce = add_light("HC_Tent_Bounce", "AREA", (-0.5, -1.0, 2.3), 130.0, (0.72, 0.78, 1.0), 3.0)
    point_at(tent_bounce, (-2.7, 1.25, 0.9))
    tent_interior = add_light("HC_Tent_Interior", "POINT", (-2.70, 1.25, 1.0), 38.0, (0.74, 0.80, 1.0))
    tent_interior.data.shadow_soft_size = 1.6
    sun = add_light("HC_Sun", "SUN", (0.0, 0.0, 10.0), 1.15, (1.0, 0.76, 0.52))
...
def write_manifest() -> None:
    payload = {
        "id": "gorski-hunters-camp-atlas-preview-v3",
        ...
        "atlas": {
            ...
            "usedTiles": ["linen-canvas", "rough-hewn-timber", "weathered-planks", "fieldstone-mortar", "wrought-iron", "wicker-weave", "packed-earth"],
            "packing": "R roughness, G metalness, B AO, A centered height",
        },
        "historicalMaterialDecision": {
            "shelters": "Off-white linen or hemp canvas over hand-cut softwood poles.",
        ...
        "runtimeOwnedState": {
            ...
            "harvestedGame": "No deer, carcass, hide, or meat mesh is included.",
...
def main() -> None:
    ...
    scene = bpy.context.scene
    scene["artifact_id"] = "gorski-hunters-camp-atlas-preview-v2"
