...
for(const agent of agents) agent.yaw=0;
...
renderer.setAnimationLoop(now=>{
 ...
 if(focus && new URLSearchParams(location.search).has('palm')){
  ...
  if(rig){
   ...
   if(!hand.userData.axes){hand.add(new THREE.AxesHelper(.15));hand.userData.axes=true;}
   const p=hand.getWorldPosition(new THREE.Vector3());
   camera.position.copy(p).add(new THREE.Vector3(.12,.1,.45));camera.lookAt(p);
  }
 }
