...
const roundRoot = resolve(
  root,
  'public/visual-gauntlet/evidence/round-54',
);
const artifactRoot = resolve(
  root,
  'artifacts/visual-gauntlet/round-54',
);
...
assert.ok(existsSync(commitmentPath), 'Round 54 commitment is missing');
...
assert.equal(
  ...
  '14DEA39B94E40D87B499D089DC75ED2E2290E6CB91A71C99EBE9848402B796C8',
);
...
assert.equal(new Set(ledger.attempts.map(({ attemptNumber }) => attemptNumber)).size, 39);
assert.deepEqual(
...
for (const attempt of ledger.attempts) {
  ...
  if (attempt.publicPath) {
    ...
    assert.ok(existsSync(publicPath), `missing retained public copy ${attempt.publicPath}`);
...
for (const blindLabel of ['blind-A', 'blind-B'] as const) {
  ...
  assert.equal(performance.renderer.medianDrawCalls, 83);
}
assert.ok(
  validation.performance['blind-B'].renderer.medianTriangles
    < validation.performance['blind-A'].renderer.medianTriangles,
  'tapered treatment should preserve its measured triangle reduction',
);
...
assert.equal(reveal.arms.control.layout, 'clustered-sapling-shrub-256');
assert.equal(
...
for (const frame of frames) {
  const blindA = resolve(roundRoot, 'blind-A', `frame-${String(frame).padStart(3, '0')}.png`);
  const blindB = resolve(roundRoot, 'blind-B', `frame-${String(frame).padStart(3, '0')}.png`);
  const control = resolve(roundRoot, 'control', `route-${String(frame).padStart(4, '0')}.png`);
  const treatment = resolve(roundRoot, 'treatment', `route-${String(frame).padStart(4, '0')}.png`);
...
const verdict = readJson<{
  ...
  gates: Record<'blind-A' | 'blind-B', {
    visual: string;
    performance: string;
  }>;
}>(resolve(roundRoot, 'blind-verdict.json'));
assert.equal(verdict.winner, 'tie');
assert.equal(verdict.confidencePercent, 95);
...
assert.equal(report.confidencePercent, 95);
...
assert.match(tracker, /Round 54 · valid evidence \/ blind tie/);
assert.match(tracker, /Round 54 · VALID EVIDENCE \/ 95% BLIND TIE/);
assert.doesNotMatch(tracker, /Round 53 · builder active/);

console.log(
  'Round 54 commitment, immutable attempts, byte-identical repeat captures, '
    + 'pose matching, blind tie, reveal, aliases, and performance bars verified.',
);
