...
try {
  ...
  if(process.argv.includes('--suite')) {
    ...
    for(const profile of (selected?[selected]:['river','inland','coastal'])) {
      await page.goto(`${origin}/water-gauntlet.html?profile=${profile}&clean=1&play=0&production=${process.argv.includes('--production')?'1':'0'}&baseline=${process.argv.includes('--baseline')?'1':'0'}`);
      await page.waitForFunction(()=>window.__WATER_GAUNTLET__,{},{timeout:120000});
      for(const view of ['near','design','far']) {
        let ev=await page.evaluate(view=>window.__WATER_GAUNTLET__.capture({view}),view);
        if(process.argv.includes('--benchmark'))ev=await page.evaluate(()=>window.__WATER_GAUNTLET__.benchmark());
        ...
        console.log(profile,view,{fps:ev.fps,gpuMs:ev.gpuMedianMs,computeMs:ev.computeGpuMedianMs,cpuMs:ev.cpuMedianMs});
      }
    }
    if(errors.length)process.exitCode=1;
  } else {
  await page.goto(`${origin}/river-lineup.html?view=near&time=6.25&clean=1&baseline=${process.argv.includes('--baseline')?'1':'0'}`);
