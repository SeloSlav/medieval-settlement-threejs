...
assert.match(placement, /This monastery already has a vineyard extension/);
assert.match(placement, /VINEYARD_MONASTERY_MAX_DISTANCE/);
assert.match(placement, /VINEYARD_MONASTERY_ADJACENCY_DISTANCE/);
...
assert.match(economy, /vineyard_parcel\(\)[\s\S]{0,100}\.building_id\(\)[\s\S]{0,60}\.find\(&monastery\.id\)/);
...
assert.match(workers, /:monastery:vineyard:center/);
