...
pub const FRESH_FOOD_COMMODITIES: [CommodityKind; 18] = [
    CommodityKind::OatGrain,
    CommodityKind::RyeBread,
    CommodityKind::MaslinBread,
    CommodityKind::Meat,
...
pub fn building_commodity_cap(kind: &str, commodity: CommodityKind) -> f64 {
    if commodity == CommodityKind::Candles {
        if let Some(capacity) = crate::devotional_candle_policy::devotional_candle_capacity(kind) {
            return capacity;
        }
    }
...
pub fn building_total_commodity_stock(building: &Building) -> f64 {
    ALL_COMMODITIES
        ...
        .copied()
        .filter(|kind| {
            *kind != CommodityKind::Gold && building_commodity_cap(&building.kind, *kind) > 0.0
        })
...
fn is_grain_food(commodity: CommodityKind) -> bool {
    matches!(
        commodity,
        CommodityKind::OatGrain | CommodityKind::RyeBread | CommodityKind::MaslinBread
    )
}
...
mod tests {
    ...
    fn shared_storage_capacity_is_authored_for_large_depots() {
        assert_eq!(
            building_shared_storage_capacity("village_storehouse"),
            Some(2500.0)
        );
    ...
    #[test]
    fn vegetable_crops_share_an_independent_food_category() {
        assert_eq!(
