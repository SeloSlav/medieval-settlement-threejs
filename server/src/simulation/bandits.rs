...
fn ensure_camps(ctx: &ReducerContext, tick: u64, seed: u64, map_size: u8) {
    ...
    for owner in owners {
        ...
        reusable.sort_by(|left, right| {
            right
                .destroyed_tick
                .cmp(&left.destroyed_tick)
                .then_with(|| right.id.cmp(&left.id))
        });
        for index in active..count {
            let recycled = reusable.pop();
            if recycled.is_none() && new_rows_available == 0 {
                break;
            }
            if recycled.is_none() {
                new_rows_available -= 1;
            }
            ...
            let camp = if let Some(mut camp) = recycled {
                ...
                camp.z = z;
                camp.health = 180.0;
                camp.max_health = 180.0;
                camp.active = true;
                camp.inventory_json = "[]".into();
                camp.spawned_tick = tick;
                camp.next_theft_tick =
                    tick.saturating_add(day_ticks().saturating_mul(theft_delay_days));
                camp.last_theft_tick = 0;
                camp.destroyed_tick = 0;
                ...
                ctx.db.bandit_camp().insert(BanditCamp {
                    ...
                    inventory_json: "[]".into(),
                    spawned_tick: tick,
                    next_theft_tick: tick
                        .saturating_add(day_ticks().saturating_mul(theft_delay_days)),
                    last_theft_tick: 0,
                    destroyed_tick: 0,
                })
            };
...
fn camp_spawn_position(
    ...
    (0..32_u64)
        .map(|candidate_index| {
            ...
            let forage_clearance = foraging
                .iter()
                .map(|node| dist(point.0, point.1, node.x, node.z))
                .fold(half * 2.0, f64::min);
            let quarry_clearance = quarries
                .iter()
                .map(|node| dist(point.0, point.1, node.x, node.z))
                .fold(half * 2.0, f64::min);
            let camp_clearance = active_camps
                .iter()
...
fn dispatch_thefts(ctx: &ReducerContext, tick: u64) {
        ...
        .collect::<Vec<_>>()
    {
        ...
        let actor = ctx.db.combat_agent().owner().filter(&camp.owner).find(|a| {
            a.faction == BANDIT && a.raid_anchor_building_id == camp.id && a.state == HOLDING
        });
...
fn steal(ctx: &ReducerContext, agent: &mut CombatAgent, tick: u64) {
    let Some(mut building) = ctx.db.building().id().find(&agent.target_id) else {
        return;
    };
    if !matches!(building.kind.as_str(), "granary" | "village_storehouse") {
        return;
    }
    ...
    if carried.goods_amount() > 0.0 {
        incident(
            ctx,
            agent.owner,
            agent.raid_anchor_building_id,
            0,
            building.id,
            &agent.carried_loot_json,
            carried.goods_amount(),
            tick,
            building.x,
            building.z,
        );
...
fn deposit(ctx: &ReducerContext, agent: &mut CombatAgent) {
    ...
    if let Some(mut camp) = ctx
        ...
        .find(&agent.raid_anchor_building_id)
    {
        let mut bundles = serde_json::from_str::<Vec<RaidPortableStores>>(&camp.inventory_json)
            .unwrap_or_default();
...
pub(super) fn destroy_camp(ctx: &ReducerContext, camp: &mut BanditCamp, tick: u64) {
    ...
    for agent in ctx
        ...
        .owner()
        .filter(&camp.owner)
        .filter(|a| a.faction == BANDIT && a.raid_anchor_building_id == camp.id)
        .collect::<Vec<_>>()
    {
        ctx.db.combat_agent().id().delete(agent.id);
    }
...
fn walk(agent: &mut CombatAgent, x: f64, z: f64, speed: f64, dt: f64) {
    ...
    let move_x = dx / d * step;
    let move_z = dz / d * step;
    agent.x += move_x;
    agent.z += move_z;
