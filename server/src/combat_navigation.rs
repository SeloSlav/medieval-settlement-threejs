...
impl CombatObstacle {
    ...
    pub fn from_corners(corners: [NavPoint; 4]) -> Self {
        ...
        Self {
            ...
            half: (
                length * 0.5 + CLEARANCE,
                distance(corners[1], corners[2]) * 0.5 + CLEARANCE,
            ),
        }
    }
    ...
    fn entry(self, start: NavPoint, end: NavPoint) -> Option<f64> {
        ...
        let mut far: f64 = 1.0;
        for (origin, delta, half) in [
            (a.0, b.0 - a.0, self.half.0 - EPS),
            (a.1, b.1 - a.1, self.half.1 - EPS),
        ] {
            if delta.abs() < EPS {
                if origin.abs() >= half {
                    return None;
...
impl CombatNavigation {
    ...
    pub fn outside(&self, mut point: NavPoint) -> NavPoint {
        for _ in 0..8 {
            let Some(obstacle) = self
                .candidates(point, point, 0.0)
                .into_iter()
                .map(|index| self.obstacles[index])
                .find(|obstacle| obstacle.contains(point))
            else {
                break;
            };
    ...
    pub fn next_waypoint(&self, start: NavPoint, goal: NavPoint) -> NavPoint {
        ...
        let local_goal = if length > 36.0 {
            self.outside((
                start.0 + (goal.0 - start.0) * 36.0 / length,
                start.1 + (goal.1 - start.1) * 36.0 / length,
            ))
        } else {
            goal
        };
        ...
        let blocked = |a, b| {
            self.candidates(a, b, 0.0).into_iter().any(|index| {
                let obstacle = self.obstacles[index];
                !obstacle.contains(start)
                    && !obstacle.contains(goal)
                    && obstacle.entry(a, b).is_some()
            })
        };
        if !blocked(start, local_goal) {
            return local_goal;
        }
        ...
        for index in candidates {
            let obstacle = self.obstacles[index];
            if !obstacle.contains(start) && !obstacle.contains(goal) {
        ...
        for _ in 0..nodes.len() {
            let current = (0..nodes.len())
                .filter(|&i| !closed[i] && cost[i].is_finite())
                .min_by(|&a, &b| {
                    (cost[a] + distance(nodes[a], local_goal))
    ...
    pub fn constrain_step(&self, start: NavPoint, goal: NavPoint, proposed: NavPoint) -> NavPoint {
        ...
        for index in self.candidates(start, proposed, 0.0) {
            ...
            if let Some(entry) = obstacle.entry(start, proposed) {
                fraction = fraction.min((entry - EPS).max(0.0));
            }
        }
        (
            start.0 + (proposed.0 - start.0) * fraction,
            start.1 + (proposed.1 - start.1) * fraction,
        )
    }
}

fn distance(a: NavPoint, b: NavPoint) -> f64 {
    ...
    fn a_company_walks_around_rotated_and_adjacent_buildings() {
        ...
        for _ in 0..240 {
            ...
            assert!(nav
                .obstacles
                .iter()
                .all(|obstacle| !obstacle.contains(point)));
            detoured |= point.1.abs() > 3.0;
            if distance(point, goal) < 0.1 {
                break;
            }
        }
        assert!(detoured);
